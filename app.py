...
CONFIG_MAP: Dict[str, str] = {
    # LAB local, tudo dummy
    "lab_dummy": "config/settings_lab_dummy.yaml",

    # Binance Testnet: dados reais + testnet para ordens
    "binance_testnet": "config/settings_binance_testnet.yaml",

    # Binance Live: dados + ordens em conta real (com vários travas)
...
def _apply_env_hardening(settings: Dict[str, Any], env_name: str) -> Dict[str, Any]:
    """
    Aplica regras de segurança adicionais dependendo do ambiente.

    - Em qualquer ambiente, se `trading.dry_run` não existe, default = True.
    - Em binance_live:
        * circuit_breaker sempre habilitado
        * dry_run só pode ser False se variáveis de ambiente de confirmação
          estiverem presentes.
    """
    risk = settings.setdefault("risk", {})
    trading = settings.setdefault("trading", {})

    # default absoluto: nunca assumimos dry_run=False por descuido
    ...
    if env_name == "binance_live":
        ...
        # Para realmente desativar dry_run em live:
        ...
        if not (allow_real and confirm):
            # Força dry_run para True mesmo que alguém coloque False no YAML
...
def load_settings(path: str | None = None) -> Dict[str, Any]:
    """
    Carrega o YAML de configuração, escolhendo o arquivo com base no APP_ENV,
    a menos que um path explícito seja fornecido.
    ...
    if path is None:
        ...
        # Se o usuário passar explicitamente um caminho, usamos ele,
        # mas ainda anotamos o env a partir de APP_ENV.
        env_name = os.getenv("APP_ENV", "lab_dummy")
        cfg_path = path
    ...
    # anota o ambiente dentro do próprio settings
    settings["env"] = env_name

    # aplica endurecimento de segurança conforme ambiente
...
def main():
    settings = load_settings()  # agora escolhe o YAML pelo APP_ENV
        ...
        for tick in datafeed.ticks():
            ...
            for ev in events:
                if ev.type == "trade_executed":
                    ...
                    logger.error("Circuit breaker disparado, encerrando loop.", extra=ev.data)
                    return
                elif ev.type == "error":
